<template>
    <div class="container flex justify-center flex-col h-full md:block mx-auto p-4 sm:mt-12 absolute bottom-12 sm:relative md:relative">
            ...
            <table class="min-w-full bg-white border border-gray-200 rounded-lg shadow-md">
                ...
                <tbody>
                    <tr v-if="paginatedProducts.length === 0">
                        <td colspan="8" class="py-3 px-4 text-gray-600 text-center">No products found.</td>
                    </tr>
                    <tr v-for="(product, index) in paginatedProducts" :key="product.SW_Id"
                        ...
                        <td>
                            <div class="flex items-center p-2 rounded-md">
                                ...
                                <p>{{ product.SW_IsApproved === 1 ? 'Approved' : 'Not Approved' }}</p>
                            </div>
                        </td>
                    </tr>
                </tbody>
        ...
        <div v-if="isDetailsModalOpen"
            ...
            <div
                ...
                <h2 class="text-2xl font-bold text-gray-800 mb-6 text-center">Product Details</h2>
                ...
                <div class="flex justify-center text-xs">
                    ...
                    <button @click="goToPage(paginateCurrentPage + 1)" :disabled="paginateCurrentPage === paginateTotalPages"
                        class="px-4 py-2 bg-blue-500 text-white rounded">
                        >
                    </button>
                </div>
                ...
                <div class="space-y-3">
                    <p class="text-gray-700"><strong>Name:</strong> <span class="font-semibold">{{
                        selectedProduct.SW_Name }}</span></p>
                    <p class="text-gray-700"><strong>In Stock</strong></p>
                    <p class="text-gray-700"><strong>Small:</strong> <span class="font-semibold">{{
                        selectedProduct.SW_Small + 'pcs' }}</span></p>
                    <p class="text-gray-700"><strong>Medium:</strong> <span class="font-semibold">{{
                        selectedProduct.SW_Medium + 'pcs' }}</span></p>
                    <p class="text-gray-700"><strong>Large:</strong> <span class="font-semibold">{{
                        selectedProduct.SW_Large + 'pcs' }}</span></p>
                    <p class="text-gray-700"><strong>Extra Large:</strong> <span class="font-semibold">{{
                        selectedProduct.SW_X_large + 'pcs' }}</span></p>
                    <p class="text-gray-700"><strong>Category:</strong> <span class="font-semibold">{{
                        selectedProduct.SW_Category }}</span></p>
                    <p class="text-gray-700"><strong>Price:</strong> <span class="font-semibold text-green-600">₱{{
                        selectedProduct.SW_Price }}</span></p>
                </div>
                <div class="flex justify-end mt-6">
                    <button @click="closeDetailsModal"
                        class="bg-gray-600 text-white px-5 py-2 rounded-lg hover:bg-gray-700 transition duration-200">Close</button>
                </div>
            </div>
        </div>
                ...
                <div v-if="isAddModalOpen">
                    ...
                    <form @submit.prevent="addProduct()">
                        ...
                        <div class="mb-4">
                            ...
                            <input v-model="name" type="text" class="w-full border border-gray-300 p-2 rounded-md"
                                required />
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div>
                                <label class="inline-flex items-center mx-2">
                                    <input type="input" v-model="stockSmall" placeholder="Small"
                                        class="w-16 p-2 border border-black rounded-md" />
                                </label>
                                <label class="inline-flex items-center mx-2">
                                    <input type="input" v-model="stockMedium" placeholder="Medium"
                                        class="w-20 p-2 border border-black rounded-md" />
                                </label>
                                <label class="inline-flex items-center mx-2">
                                    <input type="input" v-model="stockLarge" placeholder="Large"
                                        class="w-16 p-2 border border-black rounded-md" />
                                </label>
                                <label class="inline-flex items-center mx-2">
                                    <input type="input" v-model="stockXlarge" placeholder="Extra Large"
                ...
                <div v-else>
                    ...
                    <form @submit.prevent="updateProduct()">
                        ...
                        <div class="mb-4 ">
                            <label class="block text-gray-700">Category</label>
                            <label class="inline-flex items-center mx-2">
                                <input type="radio" v-model="currentProduct.SW_Category" value="shorts"
                                    class="form-radio">
                                <span class="ml-2">Shorts</span>
                            </label>
                            <label class="inline-flex items-center mx-2">
                                <input type="radio" v-model="currentProduct.SW_Category" value="tshirt"
                                    class="form-radio">
                                <span class="ml-2">T-Shirt</span>
                            </label>
                            <label class="inline-flex items-center mx-2">
                                <input type="radio" v-model="currentProduct.SW_Category" value="shoes"
                                    class="form-radio">
                                <span class="ml-2">Shoes</span>
                            </label>
                        </div>
...
const paginateTotalPages = computed(() => Math.ceil(imageObjects.value.length / itemsPerPage));
