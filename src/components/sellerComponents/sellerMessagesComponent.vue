<template>
    <div class="container md:block flex flex-col justify-center h-full mx-auto p-4 sm:mt-12 absolute bottom-12 sm:relative md:relative">
      <div class="flex flex-col md:flex-row md:justify-between items-center mb-4">
        <h1 class="text-2xl font-semibold text-gray-800 mb-4 md:mb-0">Feedbacks</h1>
        ...
        <table class="bg-white border min-w-full border-gray-200 rounded-lg shadow-md">
          ...
          <tbody>
            <tr v-if="paginatedFeedbacks.length === 0">
              <td colspan="4" class="py-3 px-4 text-gray-600 text-center">No feedbacks found.</td>
            </tr>
            <tr
            ...
            >
              ...
              <td class="py-3 px-4 text-gray-600">{{ formatDate(feedback.isCreated) }}</td>
      ...
      >
        <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md mx-auto">
          ...
          <p><strong>Date:</strong> {{ formatDate(currentFeedback.isCreated) }}</p>
  ...
  const totalPages = computed(() => Math.ceil(feedbacks.value.length / perPage.value));
