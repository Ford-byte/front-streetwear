<template>
    ...
    <div class="container flex justify-center flex-col h-full md:block mx-auto p-4 sm:mt-12 absolute bottom-12 sm:relative md:relative">
        ...
        <div class="flex flex-col md:flex-row md:justify-between items-center mb-4">
            <h1 class="text-2xl font-semibold text-gray-800 mb-4 md:mb-0">Order List</h1>
        ...
        <div class="overflow-x-auto max-h-[420px] overflow-y-auto">
            <table class="bg-white border min-w-full border-gray-200 rounded-lg shadow-md">
                <thead>
                    <tr class="bg-gray-100 border-b">
                        <th class="py-3 px-4 text-left text-gray-600 font-medium">Name</th>
                        <th class="py-3 px-4 text-left text-gray-600 font-medium">Size</th>
                        <th class="py-3 px-4 text-left text-gray-600 font-medium">Quantity</th>
                        <th class="py-3 px-4 text-left text-gray-600 font-medium">Total</th>
                        <th class="py-3 px-4 text-left text-gray-600 font-medium">Order Date</th>
                ...
                <tbody>
                    <tr v-if="paginatedOrders.length === 0">
                        <td colspan="7" class="py-3 px-4 text-gray-600 text-center">No orders found.</td>
                    </tr>
                    <tr v-for="(order, index) in paginatedOrders" :key="order.id" class="border-b hover:bg-gray-50">
                        ...
                        <td class="py-3 px-4 text-gray-600">{{ timeControl(order.SW_IsCreated) }}</td>
                        <td class="py-3 px-4 text-gray-600">
                            <div class="flex items-center p-2 rounded-md cursor-pointer"
                                ...
                                <Check v-if="order.SW_IsApproved === 1" class="text-green-500 mr-2" />
                                <Wrong v-else class="text-red-600 mr-2" />
                            </div>
                        </td>
                        <td class="py-3 px-4">
                            <button @click="openModal(order)"
                                class="bg-green-500 text-white px-2 py-1 rounded hover:bg-green-600 m-1">View
                                Details</button>
                            <button @click="confirmDelete(order.SW_Id)"
                                class="bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600 m-1">Delete</button>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <!-- Pagination Controls -->
        <div class="flex flex-row md:flex-row justify-between items-center mt-4 mb-24">
            <button @click="prevPage" :disabled="currentPage === 1"
                class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600 disabled:bg-gray-300">Previous</button>
            <span class="mt-2 md:mt-0">Page {{ currentPage }} of {{ totalPages }}</span>
        ...
        <div v-if="isModalOpen"
            ...
            <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md mx-auto">
                ...
                <p><strong>Order Date:</strong> {{ timeControl(currentOrder.SW_IsCreated) }}</p>
                <p><strong>Total Amount:</strong> {{ currentOrder.SW_Price }}</p>
                <p><strong>Location:</strong> {{ currentOrder.SW_Location }}</p>
                <p><strong>Status:</strong> {{ currentOrder.SW_IsApproved ? 'Approved' : 'Disapproved' }}</p>
                <div class="flex justify-end">
                    <button @click="closeModal" type="button"
                        class="bg-gray-500 text-white px-4 py-2 rounded mr-2 hover:bg-gray-600">Close</button>
                </div>
            </div>
        </div>
        ...
        <ConfirmationModal :visible="isDeleteModalOpen" title="Confirm Deletion"
            message="Are you sure you want to delete this order?" @confirm="deleteOrder(orderIdToDelete)"
            @cancel="closeDeleteModal" />

        <ConfirmationModal :visible="isApprovedModalOpen" title="Confirm Approval"
            message="Are you sure you want to change the approval status of this order?"
            @confirm="approvedOrder(orderIdToApprove)" @cancel="closeApprovedModal" />
