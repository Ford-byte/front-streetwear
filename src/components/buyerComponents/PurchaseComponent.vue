  <template>
    <HeaderComponent />
    <div class="bg-gray-900 fixed inset-0 overflow-auto flex justify-center items-center md:items-baseline  ">
      <div class="bg-gray-900 md:mt-24 w-full overflow-y-auto max-w-2xl">
        <h1 class="text-2xl md:text-5xl flex justify-center py-4 bg-white">My Orders</h1>
        <table class="bg-gray-800 border border-gray-700 w-full">
          ...
          <tbody class="text-white text-sm">
            <tr v-for="(order, index) in orders" :key="order.SW_Id" class="border border-white">
              <td class="py-2 px-2 text-center hidden sm:table-cell">{{ index + 1 }}</td>
        ...
        <div v-if="orders.length === 0" class="text-center bg-white p-4">No orders available.</div>
        <div v-if="loading" class="mt-4 text-center">Loading orders...</div>
      </div>
    </div>

    <section id="detailModal" v-if="isDetailsOpen"
      class="fixed top-0 left-0 w-full h-full flex justify-center items-center">
      <span class="fixed top-0 left-0 w-full h-full flex justify-center items-center bg-gray-900 opacity-50"
        @click="isDetailsOpen = !isDetailsOpen"></span>
      <div class="border p-6 bg-white text-black rounded-lg shadow-lg z-10">
        <div class="w-full justify-between flex">
          ...
          <Info class="text-red-500 hover:text-red-700 hover:cursor-pointer" :disabled="selectedOrder.SW_Status"
            @click="handleReport" />
        </div>
        <p><strong>Product Name:</strong> {{ selectedOrder.SW_Pname }}</p>
        <p><strong>Size:</strong> {{ selectedOrder.SW_Size == 'S' ? 'Small' : selectedOrder.SW_Size == 'M' ? 'Medium'
          : selectedOrder.SW_Size == 'L' ? 'Large' : 'Extra Large' }}</p>
        <p><strong>Quantity:</strong> {{ selectedOrder.SW_Quantity }}</p>
        <p><strong>Total Price:</strong> {{ selectedOrder.SW_Price }}</p>
        <p><strong>Status:</strong> {{ !selectedOrder.SW_Status ? 'Waiting for Approval' : 'Approved' }}</p>
        <p><strong>Date Ordered:</strong> {{ timeControl(selectedOrder.SW_IsCreated) }}</p>

        <div class="mt-4 flex justify-end">
          <button class="bg-red-600 text-white rounded-md px-4 py-2 mr-2"
            @click="isDetailsOpen = !isDetailsOpen">Close</button>
          <button v-if="selectedOrder.SW_Status !== 'Complete' && !selectedOrder.SW_IsApproved"
            class="bg-yellow-500 text-white rounded-md px-4 py-2" @click="openCancelModal(selectedOrder)">Cancel
            Order</button>
        </div>
      </div>

      <button class="absolute top-2 right-2 text-gray-500 hover:text-gray-800" @click="closeDetailModal()"
    ...
    <section v-if="isReportOpen" id="reportSection">
      ...
      <span class="fixed top-0 left-0 w-full h-full bg-black opacity-50" @click="isReportOpen = false"></span>
      <div class="py-4 bg-white w-full h-3/4 fixed bottom-0 overflow-y-scroll">
        <div class="container flex flex-col gap-y-2">
          <div class="flex items-center gap-4">
            <h2>Report this</h2>
            <div>
              <select name="reportType" class="border border-gray-300 rounded-md p-2" v-model="reportedPersona">
                <option :value="reportedPersona" class="mx-4">{{ reportedPersona }}</option>
          ...
          <div class="imageGrid grid grid-cols-3 gap-2">
            <div v-for="(image, index) in previewURLs" :key="index" class="relative w-full h-32 border">
          ...
          <div class="mt-4">
            <textarea v-model="message" name="reportDetails" placeholder="Describe the issue..."
              class="w-full border border-gray-300 rounded-md p-2" rows="4"></textarea>
            <div class="w-full flex justify-end gap-2 mt-2">
              <button @click="isReportOpen = false" class="bg-red-500 rounded-md px-4 py-2 text-white">Close</button>
              <button @click="submitReport" class="bg-blue-500 rounded-md px-4 py-2 text-white">Submit</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <ConfirmationModal :visible="isDeleteModalOpen" title="Confirm Deletion"
      message="Are you sure you want to delete this order?" @confirm="confirmDeleteOrder" @cancel="closeDeleteModal" />

    <ConfirmationModal :visible="isCancelModalOpen" title="Confirm Order"
      message="Are you sure you want to cancel this order?" @confirm="confirmCancelOrder" @cancel="closeCancelModal" />

    <ConfirmationModal :visible="isOrderModalOpen" title="Confirm Receipt" message="Did you receive your order?"
      @confirm="openFeedbackModal" @cancel="closeOrderModal" />

    <!-- Feedback Modal -->
    <section id="feedback" class="fixed flex justify-center items-center top-0 left-0 opacity-90 w-full h-full z-50"
      v-if="isFeedbackModalVisible">
      <span class="fixed inset-0 bg-black opacity-90 z-10" @click="closeFeedbackModal"></span>
      <div class="container bg-white flex flex-col justify-center items-center p-4 rounded shadow-lg z-40">
        <div class="text-center">
          <h2 class="text-lg font-semibold">Feedback</h2>
          <p>Your feedback is important to us!</p>
          <textarea v-model="feedbackText" placeholder="Please enter your feedback here..." rows="4"
            class="w-full border border-gray-300 p-2 rounded mt-2"></textarea>

          <div class="flex justify-center mt-2">
            <span v-for="star in 5" :key="star" @click="setRating(star)" class="cursor-pointer text-2xl"
          ...
          <button class="mt-4 px-4 py-2 mx-2 bg-blue-500 text-white rounded hover:bg-blue-600"
            @click="submitFeedback">Submit Feedback</button>
          <button class="mt-4 px-4 py-2 mx-2 bg-gray-500 text-white rounded hover:bg-gray-600"
    ...
    <SuccessModal :visible="isSuccessModalVisible" :message="successMessage" @close="closeSuccessModal" />
    <ErrorModal :visible="isErrorModalVisible" :message="errorMessage" @close="closeErrorModal" />
  </template>
...
const confirmDeleteOrder = async () => {
  ...
  try {
    ...
    orders.value = orders.value.filter(o => o.SW_Id !== selectedOrder.value.SW_Id);
...
const confirmReceivedOrder = async () => {
  try {
    ...
    errorMessage.value = 'Error marking order as received. Please try again later.';
    isErrorModalVisible.value = true;
  }
};

const closeDeleteModal = () => { isDeleteModalOpen.value = false; };
const closeCancelModal = () => { isCancelModalOpen.value = false; };
const closeSuccessModal = () => { isSuccessModalVisible.value = false; };
const closeOrderModal = () => { isOrderModalOpen.value = false; };
const closeErrorModal = () => { isErrorModalVisible.value = false; };
const closeFeedbackModal = () => { isFeedbackModalVisible.value = false; feedbackText.value = ''; };
