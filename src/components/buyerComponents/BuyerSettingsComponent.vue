<template>
    <HeaderComponent />
    <section class="w-full h-full bg-gray-900 flex justify-center items-center p-4 fixed">
        <div class="p-2 sm:p-4 sm:my-2 mb-12 lg:mb-0 bg-white text-gray-800 rounded-sm flex flex-col items-center w-full sm:w-1/2 md:w-1/3 lg:w-1/4"
            v-for="user in users.response" :key="user.SW_Id">
            <div class="w-full h-52 relative">
                <img v-if="myProfile[0]?.SW_Image" :src="getImageUrl(myProfile[0].SW_Image)" alt="User Profile Image"
                    class="w-full h-full rounded-sm object-cover" />
                <div v-else class="w-full h-full bg-gray-500 rounded flex justify-center items-center dark:bg-gray-700">
            ...
            <div class="text-center space-y-4 mt-4">
                <h2 class="text-2xl md:text-3xl font-semibold truncate">{{ userInfo[0]?.SW_Fullname || 'N/A' }}</h2>
                <div class="space-y-2">
                    <p class="text-gray-600 dark:text-gray-500 truncate"><strong>Email:</strong> {{
                        userInfo[0]?.SW_Email || 'N/A' }}</p>
                    <p class="text-gray-600 dark:text-gray-500 truncate"><strong>Cellphone Number:</strong> {{
                        userInfo[0]?.SW_Cpnum || 'N/A' }}</p>
                    <p class="text-gray-600 dark:text-gray-500 truncate"><strong>Gender:</strong> {{
                        userInfo[0]?.SW_Gender || 'N/A' }}</p>
                    <p class="text-gray-600 dark:text-gray-500 flex flex-wrap w-full text-center justify-center">
                        <strong>Location:</strong> {{ `${userInfo[0]?.SW_Street}, ${userInfo[0]?.SW_City},
    ...
    <div id="userModal" v-if="userModal"
        ...
        <div class="bg-white rounded-lg shadow-lg max-w-lg w-full z-10  overflow-auto max-h-[500px]" @click.stop>
            <div class="w-full grid grid-cols-4 gap-4 p-6">
                <div class="flex col-span-4 justify-end">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="size-6 cursor-pointer" @click="closeModal">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M9.75 9.75l4.5 4.5m0-4.5l-4.5 4.5M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                    </svg>
                </div>
                <form @submit.prevent="editInformation" class="col-span-4 grid grid-cols-4 gap-4">
                    <h2 class="text-2xl font-semibold col-span-4 mx-2">Edit Information</h2>
                    ...
                    <div class="col-span-4 sm:col-span-2">
                        <input type="text" class="w-full border p-2 rounded-lg"
                            :placeholder="'0' + userInfo[0]?.SW_Cpnum || 'Enter cellphone number'" v-model="number"
                            required />
                    </div>
