<template>
    ...
    <section class="hidden sm:block bg-black overflow-visible py-12">
        <div class="text-white items-center text-center flex flex-col mx-4">
            <h2 class="font-bold text-xl md:text-3xl lg:text-xl">Embark on an Adventure</h2>
    ...
    <section class="bg-gray-900 text-white h-full">
        ...
        <div class="flex justify-center items-center">
            <h2 class="text-3xl sm:text-5xl font-bold py-4 md:py-2">LATEST PRODUCTS</h2>
        </div>
        <div class="relative overflow-hidden mx-auto">
            <div class="carousel-wrapper sm:w-full flex transition-transform duration-700 ease-in-out"
                ...
                <div v-for="(product, index) in products" :key="product.SW_Id"
                    ...
                    <h3 class="text-xl font-semibold mb-2 uppercase text-black">{{ product.SW_Name }}</h3>
                    <p class="text-gray-700 font-semibold uppercase">{{ '₱' + product.SW_Price }}</p>
                </div>
            </div>
            ...
            <div class="modal-content flex h-full justify-center items-center z-10 inset-0">
                <div class="modal-body">
                    <div class="relative py-12">
                        <div>
                            <div v-for="product in paginatedProducts" :key="product.SW_Name">
                                <img :src="getImageUrl(product)" :alt="product.SW_Name" class="modal-image" />
                                {{ product.SW_Name }}
                            </div>
                            ...
                            <div class="pagination w-full flex justify-center items-center py-4">
                                ...
                                <button @click="goToPage(currentPage + 1)" :disabled="currentPage === totalPages"
                                    class="p-2 rounded-lg bg-blue-500 ">></button>
                            </div>
                        </div>
                    ...
                    <div class="mt-4">
                        ...
                        <div class="flex flex-wrap gap-2 mt-2">
                            <button v-for="size in productSizes" :key="size" :disabled="getSizes(size)"
                    ...
                    <div class="mt-4 flex items-center">
                        ...
                        <div class="flex items-center ml-4">
                            ...
                            <button class="bg-secondary text-secondary-foreground px-2 py-1 rounded"
                                @click="increaseQuantity" :disabled="getQuantity(selectedQuantity)">+</button>
                        </div>
                    </div>
                        ...
                        <div v-for="(review, index) in reviews" :key="index" class="mt-2">
                            ...
                            <div class="flex justify-between p-2 border rounded-lg">
                                ...
                                <div class="flex items-center ml-2">
                                    <span v-for="star in getStars(review.SW_Rating)" :key="star"
        ...
        <div class="flex justify-center m-4">
            <div class="flex flex-col max-w-3xl p-6 space-y-4 sm:p-10 dark:bg-gray-50 dark:text-gray-800 rounded-md">
                ...
                <div class="space-y-1 text-right">
                    <p>Total amount: <span class="font-semibold">₱{{ buyNowData[0].amount }}</span></p>
                </div>
                <div class="flex flex-col-reverse sm:flex-row justify-end sm:space-x-4">
                    <button @click="checkoutModal = false" type="button" aria-label="Go back to cart"
...
const totalPages = computed(() => Math.ceil(paginatedImages.value.length / imagePerPage));
...
const getProducts = async () => {
    try {
        ...
        if (Array.isArray(allProducts)) {
            ...
            const approvedProducts = allProducts.filter(product => product.SW_IsApproved === 1);
...
const fetchData = async () => {
    ...
    try {
        ...
        localStorage.setItem('location', `${userInfo.value.SW_Street}, ${userInfo.value.SW_City}, ${userInfo.value.SW_Province}`)
    } catch (error) {
...
const getSizes = (size) => {
    return size === 'S' && products.value[productIndex.value].SW_Small === 0 ? true : size === 'M' && products.value[productIndex.value].SW_Medium === 0 ? true : size === 'L' && products.value[productIndex.value].SW_Large === 0 ? true : size === 'XL' && products.value[productIndex.value].SW_X_large === 0 ? true : false;
}

const getQuantity = (qty) => {
    return mySize.value === 'S' && qty === products.value[productIndex.value].SW_Small ? true :
        mySize.value === 'M' && qty === products.value[productIndex.value].SW_Medium ? true :
            mySize.value === 'L' && qty === products.value[productIndex.value].SW_Large ? true :
                mySize.value === 'XL' && qty === products.value[productIndex.value].SW_X_large ? true : false
}
const closeModal = () => {
...
const increaseQuantity = () => {
    ...
    buyNowData.value[0].amount = buyNowData.value[0].quantity * buyNowData.value[0].price
};
...
const addOnCart = () => {
    if (selectedProduct.value && selectedSize.value) {
        try {
            cartStore.addToCart(selectedProduct.value, selectedQuantity.value, selectedSize.value);
            alert('Added to cart!');
