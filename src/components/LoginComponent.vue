<template>
  <div
    class="flex items-center justify-center w-full h-full fixed backdrop-blur-md bg-black bg-opacity-50 z-40  toast-animation-in"
    :class="animationOut == true ? 'toast-animation-out' : ''">
    <!-- Close button in the top right -->
    <span class="fixed top-0 left-0 w-full h-full z-10" @click="closeTemplate"></span>


    <!-- Login form -->
    <div class="bg-card bg-black p-6 rounded-lg shadow-lg w-full max-w-sm relative z-40">
      <div class="w-full flex justify-between items-center">
        <h2 class="text-2xl font-bold text-slate-50 text-center w-full">LOG IN</h2>
        <div @click="closeTemplate()" class="text-white cursor-pointer text-2xl z-10">
          <Closewhite />
        </div>
      </div>
      <form class="mt-4" @submit.prevent="handleLogin">
        <label class="block text-slate-50" for="username">EMAIL</label>
        <input v-model="loginData.username" class="mt-1 p-2 border border-border rounded w-full" type="text"
          id="username" placeholder="Enter your Email" required />

        <label class="block text-slate-50 mt-4" for="password">PASSWORD</label>
        <div style="position: relative; width: 100%;">
          <input v-model="loginData.password" :type="showPassword ? 'text' : 'password'"
            style="padding-right: 2.5rem; width: 100%; padding: 0.5rem; border: 1px solid #ccc; border-radius: 4px;"
            id="password" placeholder="Enter your password" required aria-label="Password"
            autocomplete="current-password" />

          <button type="button" @click="togglePasswordVisibility"
            style="position: absolute; top: 50%; right: 0.5rem; transform: translateY(-50%); background: transparent; border: none; cursor: pointer;"
        ...
        <p @click="openForgotPassword" class="text-slate-50 text-sm hover:underline mt-2 block text-right">FORGOT YOUR
          PASSWORD?</p>

        <button type="submit" class="mt-4 bg-lime-500 text-primary-foreground hover:bg-primary/80 w-full p-2 rounded">
          LOGIN
        </button>
      </form>
      ...
      <p class="mt-4 text-slate-50 text-center">
        ...
        <a @click="openRegistration" class="text-accent hover:underline">SIGN UP</a>
      </p>
    </div>
  </div>
  ...
  <ErrorModal :visible="showErrorModal" :message="errorMessage" @close="showErrorModal = false" :timeout="1000" />
...
const emit = defineEmits(["closeTemplate", "openRegistration", "openForgotPassword"]);
