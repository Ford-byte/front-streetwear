<template>
  <div class="container mx-auto p-4">
    ...
    <div class="flex flex-col md:flex-row md:justify-between items-center mb-4">
      <h1 class="text-2xl font-semibold text-gray-800 mb-4 md:mb-0">Reports List</h1>
      ...
      <table class="min-w-full bg-white border border-gray-200 rounded-lg shadow-md">
        <thead>
          <tr class="bg-gray-100 border-b">
            <th class="py-3 px-4 text-left text-gray-600 font-medium">Reporter name</th>
            <th class="py-3 px-4 text-left text-gray-600 font-medium">Message</th>
            <th class="py-3 px-4 text-left text-gray-600 font-medium flex justify-center">Action</th>
          </tr>
        </thead>
        <tbody>
          <tr v-if="paginatedReports.length === 0">
            <td colspan="4" class="py-3 px-4 text-gray-600 text-center">No reports found.</td>
          </tr>
          <tr v-for="(report, index) in paginatedReports" :key="index" class="border-b hover:bg-gray-50">
            ...
            <td class="py-3 px-4">
              <button @click="openReportModal(report)"
                class="bg-blue-500 text-white px-2 py-1 rounded hover:bg-blue-600 m-2">View</button>
              <button @click="confirmDelete(report.reportId)"
    ...
    <div class="flex justify-between items-center mt-4">
      <button @click="prevPage" :disabled="currentPage === 1"
        class="bg-transparent text-white px-4 py-2 rounded hover:bg-gray-600 disabled:bg-gray-300">Previous</button>
      <span>Page {{ currentPage }} of {{ totalPages }}</span>
      <button @click="nextPage" :disabled="currentPage === totalPages"
        class="bg-transparent text-white px-4 py-2 rounded hover:bg-gray-600 disabled:bg-gray-300">Next</button>
    </div>

  </div>

  <!-- Report Modal -->
  <div v-if="reportModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50"
    @click.self="closeReport">
    <div class="bg-white p-6 rounded-lg max-w-[1000px] max-h-[500px] shadow-lg relative z-10">
      ...
      <div class="mb-4 flex justify-between">
        ...
        <img :src="getImageUrl(selectedReport.images)" alt="profile" class="size-12 rounded-full m-2">

      </div>


      <!-- Image Carousel (if images are available) -->
      <div v-if="imageArray.length > 0" class="relative w-full h-64 rounded overflow-hidden">
        <img :src="getImageUrl(imageArray[currentImageIndex])" alt="Slide Image" class="w-full h-64 object-cover" />
        ...
        <div class="absolute top-1/2 left-0 right-0 flex justify-between">
          ...
          <button @click="nextImage" :disabled="currentImageIndex === imageArray.length - 1"
            class="bg-gray-500 text-white px-3 py-3 rounded-full hover:bg-gray-400 disabled:bg-gray-300">
      ...
      <div class="flex justify-end">
        <button @click="closeReport" class="bg-gray-500 text-white px-4 py-2 rounded-md hover:bg-gray-600 transition">
          Close
        </button>
      </div>
    </div>
  ...
  <ConfirmationModal :visible="isDeleteModalOpen" title="Confirm Deletion"
    message="Are you sure you want to delete this report?" @confirm="deleteReport(userIdToDelete)"
...
const totalPages = computed(() => Math.ceil(reportsCount.value / perPage.value));
